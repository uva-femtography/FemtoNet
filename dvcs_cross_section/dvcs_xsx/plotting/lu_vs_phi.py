...
def parse_args():
    ...
    parser.add_argument("-model",
                        required=False,
                        type=str,
                        default='dvcs_xsx/saves/femtonet_LU_standard_0/')
...
def main():
    ...
    xbj = np.expand_dims(np.array([args.xbj for i in range(361)]), 1)
    t = np.expand_dims(np.array([args.t for i in range(361)]), 1)
    Q2 = np.expand_dims(np.array([args.Q2 for i in range(361)]), 1)
    k0 = np.expand_dims(np.array([args.k0 for i in range(361)]), 1)
    phi = np.expand_dims(np.array(np.arange(361) + 0.5), 1)
    ...
    dset = dvcs_xsx.data.keep_observables(dset, [2], column=5)
    ...
    ax.set_ylabel(r"$\sigma_{LU}$ (nb/GeV$^4$)", fontsize=22, fontname="Times")
